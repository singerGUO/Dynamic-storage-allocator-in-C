...
void* mm_malloc (size_t size) {
 ...
 // size_t precedingBlockUseTag;
  ...
  //first we are looking for the Free block with the request size
  ptrFreeBlock = searchFreeList(reqSize);
  //If we can't find it for the first time  we need to request more space
  // from the heap the nget the pointer of FREE_List_HEAD
  ...
  // Get the size of the block includidng the header
  blockSize = SIZE(ptrFreeBlock->sizeAndTags);

  // Split the block if the rest of the unallocated block is more than 32byte(header,prev*,next*,footer)
  if (blockSize< MIN_BLOCK_SIZE+reqSize) {

    // our pointer goes tothe start of the next block
      BlockInfo* nextBlock = (BlockInfo*) UNSCALED_POINTER_ADD(ptrFreeBlock, blockSize);
    // Set the next block's previous allocated tag to be 1
    nextBlock->sizeAndTags = nextBlock->sizeAndTags | TAG_PRECEDING_USED;

    // Set the block's allocated tag to be 1
    ptrFreeBlock->sizeAndTags =  ptrFreeBlock->sizeAndTags | TAG_USED;
    // Remove the block from the free list
      removeFreeBlock(ptrFreeBlock);
  } else {

 // Set the block's perceding_allocated bit and tag_allocated bit to be 1
      ptrFreeBlock->sizeAndTags = reqSize | TAG_PRECEDING_USED|TAG_USED;


    // Set the remaining free block's preceding used tag to be 1
    ...
    // Set the preceding used tag in the footer of the remaining free block to be 1
       BlockInfo* footer=(BlockInfo*)UNSCALED_POINTER_ADD(ptrFreeBlock,blockSize-WORD_SIZE);
       footer->sizeAndTags = (blockSize-reqSize) | TAG_PRECEDING_USED;


    // Insert the remaining free block back to the list
  ...
  // return the ptr which is the second block from the left
...
void mm_free (void *ptr) {
  ...
  blockSize = SIZE(blockInfo->sizeAndTags);
   // Set the block's allocated tag to 0

        blockInfo->sizeAndTags = blockInfo->sizeAndTags & (~TAG_USED);
  //when we are freeing the list we need to care about its footer as well
   BlockInfo* footer=(BlockInfo*)UNSCALED_POINTER_ADD(blockInfo,blockSize-WORD_SIZE);
   // footer and header for this freed block are all equal to each other
       footer->sizeAndTags = blockInfo->sizeAndTags;



  // Get the pointer to the following block by adding the payloadsize
  followingBlock = (BlockInfo*) UNSCALED_POINTER_ADD(blockInfo, blockSize);
  // Set the following block's preceding allocated bit
  followingBlock->sizeAndTags = followingBlock->sizeAndTags & (~TAG_PRECEDING_USED);


 // Insert the block be freed to the free list
  insertFreeBlock(blockInfo);
  // we don't need to care about the fllowing block'sfooter because Coalesce the free blocks consider that for us
